...
class Solution:
    ...
    def twoSumHashMap(self, nums: List[int], target: int) -> List[int]:
        d= {key:val for val,key in enumerate(nums)}
        for i in range(len(nums)-1):
...
s = Solution()
s.twoSumHashMap(nums = [2,7,11,15], target = 9) # Output: [0,1]
s.twoSumHashMap(nums = [3,2,4], target = 6) # Output: [1,2]
s.twoSumHashMap(nums = [3,3], target = 6) # Output: [0,1]